---
name: ascend-optimization
description: 华为昇腾 Ascend 上通用 PyTorch 优化。用于任意仓库中的 torch_npu 亲和算子替换、推理瓶颈定位后的代码级优化、host-bound 场景的流水优化、Stream 级 TaskQueue 并行下发、CPU 绑核、内存分配器调优，以及同输入同精度的 baseline/perf 验证。优先适用于没有固定目录结构或专项验证脚本的通用仓库；当仓库已经具备 `model_files/`、`accuracy_run.py`、`accuracy_run_perf.py`、`check_accuracy_run_perf.py`、状态流转或专项验收脚本时，同时参考同仓库中的 `ascend-affinity-operator` skill。触发词：Ascend 优化、ascend-optimization、torch_npu 优化、CPU_AFFINITY_CONF、PER_STREAM_QUEUE、TASK_QUEUE_ENABLE、tcmalloc、PYTORCH_NPU_ALLOC_CONF、accuracy_run_perf。
---
