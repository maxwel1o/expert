...
- **self**（`Tensor`）：必选参数，拷贝目标张量（即接收数据的张量）。
- **src**（`Tensor`）：必选参数，拷贝源张量（即提供数据的张量）。
- **non_blocking**（`bool`）：可选参数，默认值为`False`。如果设置为`True`，则拷贝可能相对于主机异步发生。在其他情况下，此参数没有效果。

## 约束说明

copy*memory*仅支持NPU张量。copy*memory*的输入张量应具有相同的dtype和设备index。
