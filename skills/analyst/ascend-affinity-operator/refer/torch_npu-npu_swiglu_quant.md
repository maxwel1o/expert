...
- **smooth_scales** (`Tensor`)：可选参数，表示smooth量化系数。数据类型支持`float32`，支持非连续的`Tensor`，数据格式为$ND$。shape支持[G, N]，[G, ]。
...
- **group_index** (`Tensor`)：可选参数，当前支持`cumsum`和`count`两种模式，要求为1维张量，数据类型支持`int32`，数据格式$ND$，shape支持[G, ]，`group_index`内元素要求为非递减，且最大值不得超过输入`x`的除最后一维之外的所有维度大小之积。
- **activate_left** (`bool`)：可选参数，swiglu流程中是否进行左激活，默认值为`False`。
- **quant_mode** (`int`)：可选参数，表示量化类型，默认值为`0`。`0`表示静态量化，`1`表示动态量化。
- **group_list_type** (`int`)：可选参数，表示`group_index`类型，默认值为`0`。`0`表示`cumsum`模式，`1`表示`count`模式。
- **dst_type** (`ScalarType`)：可选参数，表示输出量化类型，支持`int8`和`int4`，传`None`时视为`int8`处理，默认值为`None`。
