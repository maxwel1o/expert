...
- **max_grad_norm** (`Scalar`)：数据类型与`var`相同。Shape为\(1, \)。
- **global_grad_norm** (`Scalar`)：数据类型与`var`相同。Shape为\(1, \)。
- **weight_decay**(`Scalar`)：数据类型与`var`相同。Shape为\(1, \)。
- **step_size** (`Tensor`)：可选参数，数据类型与`var`相同。Shape为\(1, \)。默认值为None。
